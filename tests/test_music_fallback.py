...
async def test_fallback_player_prefers_lavalink(monkeypatch):
    ...
    player = FallbackPlayer(
        ...
        search_cache=SearchCache(persist=False),
    )
...
async def test_fallback_player_reuses_cache(monkeypatch):
    ...
    backend.responses["broken"] = failing
    backend.responses["https://cached"] = [make_handle("cached")]
    ...
    assert first_entry.is_fallback
    assert cache.size() == 1
    assert call_counter["yt"] == 1

    backend.calls.clear()
    backend.responses["broken"] = AssertionError("unexpected lavalink usage")

    second_entry = await player.build_queue_entry(
        "broken", requested_by=1, requester_display="tester", channel_id=1
    )
    assert second_entry.is_fallback
    assert call_counter["yt"] == 1
    assert backend.calls
    assert backend.calls[0][0] == "https://cached"
    assert not any(call[0] == "broken" for call in backend.calls)
