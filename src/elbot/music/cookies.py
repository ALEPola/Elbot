...
class CookieManager:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self._export_interval = 60.0
        try:
            self._export_timeout = float(os.getenv("YTDLP_COOKIES_EXPORT_TIMEOUT", "30"))
        except ValueError:
    ...
    def _determine_default_cookie_path(self) -> Path:
        ...
        if data_dir:
            return Path(data_dir).expanduser().resolve() / "yt-cookies.txt"
        ...
        with self._lock:
            ...
            for candidate in (self.env_var, *self._alt_env_vars):
                ...
                if self._path and self._auto_export_enabled:
                    # Managed by auto-export; keep watching the same target
                    pass
    ...
    def _maybe_auto_export(self, target: Path) -> None:
        ...
        def _run_export() -> None:
            success = self._export_cookies(target)
            with self._lock:
    ...
    def _export_cookies(self, target: Path) -> bool:
        ...
        try:
            ...
            first_line = snippet.splitlines()[0] if snippet else "<no output>"
            _logger.warning(
        ...
        if extracted_line:
            _logger.info(
                "Auto-exported YouTube cookies via %s -> %s (%s)",
                browser_spec,
                target,
                extracted_line,
            )
            ...
            _logger.info(
                "Auto-exported YouTube cookies via %s -> %s",
                browser_spec,
                target,
            )
        return True
